...
function New-UpdateFixture {
	...
	$statePath = Join-Path $root 'last-successful-deployment.json'
	if (-not $NoState) {
...
function New-FakeUpdateRunner {
	...
	$runner = {
		...
		if ($Operation -in @('docker-compose-up-candidate', 'docker-compose-up-prior')) { $entry.environment_text = Get-Content -LiteralPath $Arguments[2] -Raw }
...
function New-FakeHealthRunner {
	param([hashtable] $Failures, [Collections.Generic.List[string]] $Trace)
	$state = @{ calls = 0 }
	$runner = {
		$state.calls++; $Trace.Add('health')
		if (($Failures.ContainsKey('health') -and $state.calls -eq 1) -or ($Failures.ContainsKey('rollback-health') -and $state.calls -gt 1)) { return @{ healthy = $false; local_health = $false } }
...
function Invoke-UpdateFixture {
	param([hashtable] $Failures = @{}, [switch] $NoState)
	$fixture = New-UpdateFixture -NoState:$NoState; $trace = [Collections.Generic.List[string]]::new()
	try {
		$result = Invoke-TideBotProductionUpdate -RepositoryPath $fixture.root -StateRoot $fixture.root -StatePath $fixture.state_path -ComposeFile $fixture.compose_path -CommandRunner (New-FakeUpdateRunner $fixture $Failures $trace) -HealthRunner (New-FakeHealthRunner $Failures $trace) -Synthetic -SkipLock
...
try {
	...
	Assert-Trace $success.trace @('git-fetch-tag', 'git-resolve-deployable', 'git-test-ancestor-main', 'git-status-clean', 'git-switch-detach', 'git-head', 'docker-inspect-current-image', 'docker-inspect-current-labels', 'docker-archive-volume', 'docker-list-archive', 'docker-build-candidate', 'docker-compose-up-candidate', 'health') 'controlled successful deployment'
	...
	Assert-Trace $partial.trace @('git-fetch-tag', 'git-resolve-deployable', 'git-test-ancestor-main', 'git-status-clean', 'git-switch-detach', 'git-head', 'docker-inspect-current-image', 'docker-inspect-current-labels', 'docker-archive-volume', 'docker-list-archive', 'docker-build-candidate', 'docker-compose-up-candidate', 'docker-compose-down', 'docker-list-archive', 'docker-restore-volume', 'docker-compose-up-prior', 'health') 'partial replacement recovery'
	...
	Assert-True ($entryOutput -match 'last-successful-deployment.json') 'Normal script entry point did not preserve its StatePath default.'
